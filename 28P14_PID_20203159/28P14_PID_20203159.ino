...
// Distance sensor
#define _DIST_ALPHA 0.3  //ema 필터의 alpha 값을 0.3으로 설정

// Servo range
#define _DUTY_MIN 1350    //서보의 가동 최소 각도(0)
#define _DUTY_NEU 1570    //servo neutral position (90 degree)
#define _DUTY_MAX 2000    //서보의 최대 가동 각도(180º)

// Servo speed control
#define _SERVO_ANGLE 30   //레일플레이트가 사용자가 원하는 가동범위를 움직일때, 이를 움직이게 하는 서보모터의 가동범위
#define _SERVO_SPEED 200   //서보 속도
...
// PID variables
float error_curr=0, error_prev=0, control, pterm, dterm, iterm; //비례 제어를 위한 전에 측정한 오차, 새로 측정한 오차 값, 비례항, 적분항, 미분항 변수
...
////////////////////
  ...
  // duty_target = f(duty_neutral, control)
    duty_target = (float)_DUTY_NEU + control;
...
//    else duty_target = (float)(_DUTY_MAX-_DUTY_NEU)/((225-10)-0) * control + (float)_DUTY_NEU;
  ...
  // keep duty_target value within the range of [_DUTY_MIN, _DUTY_MAX]
    ...
    if (duty_target > _DUTY_MAX) duty_target = _DUTY_MAX; // duty_target > _DUTY_MAX 일 때 duty_target 를 _DUTY_MAX 로 고정
...
const float coE[] = {-0.0000050, 0.0052295, 0.1008480, 71.3854721};
